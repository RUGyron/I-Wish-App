...
private final class Fetcher: NSObject, WKNavigationDelegate {
    ...
    private func attachToWindow(_ wv: WKWebView) {
        // В Share Extension нет UIApplication.shared. WKWebView работает без attach,
        // хоть и менее надёжно для JS-heavy страниц. Для антибот Ozon/Я.Маркет в extension
        // всё равно не пробивается — для них result будет nil, юзер заполнит вручную.
        _ = wv
    }
